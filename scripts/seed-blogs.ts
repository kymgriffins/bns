...
if (!supabaseServiceKey) {
  console.error("Error: SUPABASE_SERVICE_ROLE_KEY or SUPABASE_SERVICE_KEY not set in environment");
  process.exit(1);
}
...
async function seed() {
  ...
  try {
    ...
    console.log(`✅ Successfully seeded ${seedData.length} blog posts!`);
    console.log("\nSeeded entries:");
    seedData.forEach((post, index) => {
      console.log(`  ${index + 1}. ${post.title}`);
    });
    console.log("\n📝 These posts are now available at /news and /blogs");
