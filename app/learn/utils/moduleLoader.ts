...
export type SlideContent =
  ...
  | {
      headline: string;
      stats: { emoji: string; label: string; value: string; desc: string }[];
    }
  | {
      headline: string;
      sub: string;
...
export type ModuleMetadata = {
  ...
  structure: {
    ...
    stories?: {
      // Matches `app/learn/data/modules/<id>/metadata.json` `structure.stories`
      title?: string;
      description?: string;
      duration?: string;
      level?: "stories" | string;
      slides?: number;
      isActive?: boolean;
      icon?: string;
      order?: number;
...
export async function loadModuleMetadata(moduleId: string): Promise<ModuleMetadata | null> {
  try {
    // In the browser, prefer the JSON-backed API so the viewer always follows the same contract.
    ...
    if (!response.ok) return null;
    return (await response.json()) as ModuleMetadata;
  } catch {
...
export async function loadModuleSlides(
  ...
  try {
    // In the browser, prefer the API contract (and any overrides) over direct JSON fetches.
    if (typeof window !== "undefined") {
      const response = await fetch(`/api/learn/modules/${moduleId}/slides?level=${level}`);
      if (!response.ok) return null;
      const json = await response.json();
      return json?.data?.slides ?? null;
    }
 ...
 */
export function convertToHubModule(
  metadata: ModuleMetadata,
  level: "basic" | "advanced" | "stories"
): HubModule | null {
  const levelData =
    level === "basic"
      ? metadata.structure.basic
      : level === "advanced"
        ? metadata.structure.advanced
        : metadata.structure.stories;

  if (!levelData) return null;

  const slidesCount =
    typeof levelData.slides === "number"
      ? levelData.slides
      : typeof levelData.basicCount === "number"
        ? levelData.basicCount
        : 0;

  return {
    id: level === "basic" ? metadata.id : `${metadata.id}-${level}`,
    num: metadata.num,
    title: levelData.title ?? metadata.title,
    level,
    desc: levelData.description ?? metadata.description,
    duration: levelData.duration ?? "",
    slidesCount,
    lessonsCount: slidesCount || 0,
    videosCount: 0,
    quizCount: 0,
